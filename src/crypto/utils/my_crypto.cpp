...
namespace utils {
	...
	int Base58::Decode(const std::string &strIn, std::string &strout) {
		...
		return (int)(nZeros + tmp_str.size() - k);
	}
...
#include <openssl/md5.h>
	...
	std::string Aes::Decrypto(const std::string &input, const std::string &key) {
		...
		AES_set_decrypt_key((const unsigned char *)key.c_str(), (int)key.size() * 8, &dec_key); // Size of key is in bits
	...
	std::string Aes::Crypto(const std::string &input, const std::string &key) {
		...
		AES_KEY enc_key;
		AES_set_encrypt_key((const unsigned char *)key.c_str(), (int)key.size() * 8, &enc_key);
	...
	void AesCtr::Encrypt(unsigned char *indata, unsigned char *outdata, int bytes_read) {
		...
		AES_set_encrypt_key((const unsigned char *)ckey_.c_str(), (int)ckey_.size() * 8, &key);
